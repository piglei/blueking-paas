...
CONTROL_PLANE_TIMEOUT_SECONDS = env.float(
    "CONTROL_PLANE_TIMEOUT_SECONDS", 30.0, validate=Range(min=0)
)
...
if CONTROL_PLANE_URL and not CONTROL_PLANE_TOKEN:
    raise EnvError(
        f"{ENV_PREFIX}CONTROL_PLANE_TOKEN must be set whenever {ENV_PREFIX}CONTROL_PLANE_URL is"
    )
