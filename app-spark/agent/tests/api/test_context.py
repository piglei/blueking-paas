...
def test_a_restored_runtime_continues_the_conversation_numbering(api: TestClient) -> None:
    ...
    assert [record["seq"] for record in drain_channel(api, "/log")][0] == 41
    assert [record["seq"] for record in drain_channel(api, "/ui-events")][0] == 56
...
def test_a_restore_without_cursors_starts_the_numbering_at_one(api: TestClient) -> None:
    ...
    assert [record["seq"] for record in drain_channel(api, "/log")][0] == 1
