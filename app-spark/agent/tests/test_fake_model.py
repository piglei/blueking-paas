...
def wrote_file(filename: str) -> tuple[ModelResponse, ModelRequest]:
    ...
    call = ModelResponse(
        parts=[ToolCallPart(tool_name=WRITE_FILE_TOOL, args={"path": filename}, tool_call_id="c1")]
    )
    result = ModelRequest(
        parts=[ToolReturnPart(tool_name=WRITE_FILE_TOOL, content="ok", tool_call_id="c1")]
    )
    return call, result
