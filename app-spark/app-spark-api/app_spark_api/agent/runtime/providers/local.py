...
class LocalProcessProvider(AgentRuntimeProvider):
    ...
    async def _spawn(
        ...
        *,
        ...
        try:
            await self._wait_until_healthy(base_url, process, log_path)
        ...
        return _LocalRuntime(
            handle=AgentRuntimeHandle(conversation_id=conversation_id, base_url=base_url),
            process=process,
            workspace_dir=workspace_dir,
            log_path=log_path,
        )
    ...
    def _build_env(self, *, workspace_dir: Path, state_dir: Path) -> dict[str, str]:
        ...
        if self.config.api_key is not None:
            env[f"{ENV_PREFIX}API_KEY"] = self.config.api_key
    ...
    async def _wait_until_healthy(
        ...
        async with httpx2.AsyncClient(timeout=HEALTH_PROBE_TIMEOUT_SECONDS) as client:
            while True:
                try:
